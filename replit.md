...
- Pro entitlement is enforced server-side, not by a client flag: the client stores a verified Stripe checkout session id (`sm_session_id`) and sends it as header `x-sm-session-id`; the server verifies an active/trialing subscription via Stripe (`stripeService.verifyProAccess`, short in-memory cache, no DB table) before granting Pro. The 2-free-generation trial stays a client-side soft limit; `express-rate-limit` (in-memory, `skip` when `req.isPro`) on `/api/timing-map` and `/api/products/scan-label` is the abuse backstop. Requires `app.set("trust proxy", 1)`. Limits tunable via `TIMING_MAP_RATE_LIMIT` / `SCAN_LABEL_RATE_LIMIT`. Accepted no-auth tradeoffs: the session id is a bearer credential (shareable until the subscription lapses), and in-memory stores are per-instance (not global on autoscale) — see `.agents/memory/server-verified-pro-entitlement.md`.
